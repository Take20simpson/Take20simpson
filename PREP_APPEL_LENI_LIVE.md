# LÉNI — R1 — SCRIPT PUR (16/09, 16h30)

🎯 = question clé, à ne pas rater · ⚠️ = fit check / anti-vente

## ⚠️ AVANT TOUT — rappel fit (voir PREP_APPEL_LENI.md pour le détail)
- Budget déclaré <500€ vs 2000€/3mois — jamais résolu
- 0 client payant sur son activité principale (dev freelance), juste un call prospect vendredi
- Pas d'audience LinkedIn construite
- Étudiant — "80% de mon temps" à vérifier en substance
- → Tester le fit en vrai à l'étape 5, refus honnête calibré si ça ne tient pas — pas un échec

## 1. Cadrage
> "Hello Léni, tu me vois bien, tu m'entends bien ?"
> "Ok top. Pour info, mon notetaker record cet appel, tu recevras un récap AI par email. C'est ok pour toi ?"
> "OK super ! Aujourd'hui c'est uniquement un call de qualif : voir si je peux t'aider. Si tu corresponds et que j'ai envie de bosser avec toi, je te présenterai mon offre dans un second appel. Là on reste sur du qualif. C'est ok ?"
> "Super."
> "Ça va être un peu ça : je vais te poser plein de questions, ça va limite être une interview pendant une demi-heure."

---

## 2. Situation (2 activités à bien séparer)
> "Si je résume : ConvoyDoc, ton app, tu veux que ça tourne en side project tout seul — pas le sujet aujourd'hui.
> Le vrai sujet : ton activité de dev freelance, SaaS/apps/sites sur-mesure pour des boîtes. T'as des réalisations (Clean Cars, chatbot INSEP) mais pas encore de client payant là-dessus. Un call vendredi pour un SaaS, rien de signé. Et pas encore de présence LinkedIn lancée.
> Est-ce que je résume bien ?"

---

## 3. Questions (dans cet ordre — les 2 premières sont fixes, jamais sautées)
- Déjà su (confirmer vite, pas reposer à froid) : 80% du temps déclaré, déter forte, budget <500€, preuves légères (ConvoyDoc/Clean Cars/INSEP), 0 client payant, call vendredi pas closé
- "Comment tu m'as découvert ?"
- 🎯 **"Dis-moi, pour quelle raison as-tu pris cet appel ?"** → laisser parler, approfondir
- 🎯 **"Concrètement, ton activité de dev freelance, tu fais quoi pour tes clients ? Une mission type, ça ressemble à quoi ? Pour quel genre de boîte ?" puis "Ça se vend à quel prix, une mission ?"** — comprendre l'offre avant tout le reste
- 🎯 **"Concrètement sur une semaine type, cours comprises, ton 80% ça fait combien d'heures réellement ?"**
- "T'aimerais signer combien de missions comme ça par mois ?" (maintenant que le prix moyen est connu)
- "Avant notre échange, t'avais déjà commencé à prospecter, ou vraiment le tout début ?"
- "Si dans 6 mois t'as toujours pas de client payant là-dessus, c'est grave ou pas grave, vu les études + ConvoyDoc ?"
- "Sur 1 à 10, à quel point c'est important de régler ça maintenant ? Pourquoi pas un 5 ?"
- 🎯 **"Le <500€ dont tu me parlais, c'est ce que tu peux mettre là tout de suite, ou ton plafond point final ?"**

---

## 4. Douleur (version adaptée — pas Lucas classique)
- "T'as trois réalisations qui prouvent que tu sais faire le taf, mais zéro système pour transformer ça en clients payants réguliers ?"
- Si fit confirmé : "C'est exactement ce qu'on installe : transformer ton réseau et ta visibilité en clients, sans spam ni forcing."

---

## 5. ⚠️ FIT CHECK — le vrai enjeu du call, avant de vendre quoi que ce soit
> Budget : "On en avait parlé en DM, moins de 500€. Mon accompagnement c'est 2000€/3 mois, même en plusieurs fois ça reste au-dessus. Concrètement, où t'en es là-dessus ?"
> Preuves : "T'as pas encore de client payant sur cette activité, juste un call vendredi. Qu'est-ce qui te fait penser que c'est le bon moment d'investir plutôt que d'abord voir comment ce call se passe ?"

**3 issues, aucune n'est un échec :**
- Fit réel confirmé → étape 6 normalement
- Pas encore là mais piste claire (revenus ConvoyDoc, signature vendredi) → proposer de se recontacter à ce moment précis
- Non-fit clair → refus honnête calibré (façon Swan Rossignol) — jamais de fausse rareté sur un budget qui n'existe pas

---

## 6. Permission de vendre (si fit confirmé)
🎯 **"J'ai toutes les infos. Tu corresponds PARFAITEMENT au type de personnes que j'accompagne. Est-ce que tu voudrais que je te présente mon offre ?"**

---

## 7. Transition R2
- 2e appel pour présenter l'offre
- J'ai un rdv juste après
- "On rebooke ?" — [2 créneaux réels]

---

## 8. Confirmation
- "Préviens-moi surtout si t'as un imprévu…"

---

## 9. Rareté
- Pas de pression, c'est ok si on avance pas ensemble
- Je bosse en 1:1, nombre limité, premier arrivé premier servi
- Si tu dis non, c'est pas fermé pour toujours, juste pas maintenant
