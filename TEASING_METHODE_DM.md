...
8. **Varier la FORME du reopener a travers un batch de relances traitees dans la meme session (ajoute le 27 juillet 2026, demande de Matthias sur un "gros travail de reprise" de plusieurs conversations abandonnees a la suite).** La regle 2 (rebondir sur un element specifique) garantit un ancrage different a chaque fois, mais pas une forme differente — le risque reel, deja documente ailleurs pour les reponses normales (`SKILL_SETTING_DM.md`, piege recurrent "validation courte + question"), c'est de resservir le meme mecanisme (typiquement le callback humoristique "ca m'a fait penser a toi") a tout le monde, meme avec un contenu different a chaque fois. Ca redevient un script detectable, l'inverse de l'effet "decontracter l'atmosphere" recherche.
   **Reflexe correctif :** avant chaque reopener d'un batch, se demander explicitement quelle FORME a deja ete utilisee sur les relances precedentes du meme lot, et en choisir une differente. Formes disponibles a alterner (liste non figee, a enrichir au fil des cas reels) :
   - **Callback humour classique** — un element externe/quotidien qui rappelle un running gag de LEUR conversation (l'exemple historique "infographie illisible... pense a toi et ton coup des vieux")
