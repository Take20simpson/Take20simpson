...
> v3 finalisée en juin 2026 après 4 tests calibrés en live avec Matthias (post playbook,
> post société/agressions, post jargon compta, post rage-bait enfants/argent).
> Tout ce qui est marqué VALIDÉ ou REJETÉ ci-dessous vient directement de ses verdicts.
...
~10-15 mots, une seule idée, pas de subordonnée qui fait perdre le fil. Tout développement
part dans la phrase suivante.
...
- "un gosse encaisse mieux des parents fauchés que des parents paniqués"
  → distinction en mots concrets (fauchés/paniqués), pas en concepts
...
D'abord un mini-bloc d'analyse (registre détecté, postures choisies, risques éventuels),
puis le bloc question de fin, puis les 4 options en bloc code, posture indiquée entre
parenthèses (pour Matthias, pas à coller).
...
- Une structure syntaxique unique répétée à chaque session — varier la syntaxe du pont
  (cause → effet, comparaison de deux états, "ça vaut pour X comme pour Y", conséquence
  dans le temps...)
...
ou une autre anecdote. (Règle née en test : le bac 10,5 collait à deux posts d'affilée,
il a été bloqué sur le deuxième.)
...
- A payé un coaching cher (en 3x, ses dernières économies de l'époque) : templates à
  ...
  résultat *(utilisé récemment en test)*
...
- Bac à 10,5, traité de "fainéant", scolarité à croire qu'il était le problème — tout
  appris seul, en autodidacte *(utilisé récemment en test — validé "absolument génial")*
...
- ZÉRO tiret long (—), remplacer par virgule
...
Une question collable à la fin de n'importe laquelle des 4 options.
...
- Tiret long, hashtag, émoji hors registre, point final
- Négation corrective en ouverture des options 1-3 — seule l'option 4 challenge

---

## CHECKLIST FINALE (avant de sortir la série)

1. **Test du lecteur** : chaque option est plus pertinente que le post lui-même ?
2. **Les 5 lois de la première phrase** : courte + autonome + orale + concrète + jamais récitée — sur les 4 options ?
3. **4 musiques d'ouverture différentes** dans la série ?
4. **Miroir** : les 4 options épousent le registre du post ?
5. **4 postures différentes**, longueurs visiblement différentes ?
6. **Ponts** : mécanisme pur, cible jamais nommée, syntaxe variée — ou désactivés si post sensible ?
7. **Adressage** : on parle À l'auteur ("tu", "ton post"), jamais en observateur ?
8. **Clivant** : tension d'abord, "je" après, jamais contre l'auteur d'un témoignage ?
9. **Vécu** : tout détail précis vient de la banque, aucune anecdote utilisée récemment ?
10. **Zéro recyclage** de formulations, tics justifiés par le contexte, 0-2 imperfections ?
11. **Post sensible** : lecture stratégique faite, noyau défendable, risque signalé ?
