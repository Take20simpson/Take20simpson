...
- **Garantie (mise a jour le 17 septembre 2026) :** prolongation sans surcout jusqu'a deux (2) clients minimum signes grace a la prospection (souvent plus), si le client a respecte ses engagements (cadence, suivi, presence, application) — remplace l'ancienne version "jusqu'au premier client signe"
...
**Analyse de setting — méthode Yann (créé le 1er septembre 2026, raccourci le même jour — je trouvais les réponses trop longues, je veux du direct et actionnable) :**
```
On continue l'analyse de mon setting DM, méthode Yann.

LECTURE OBLIGATOIRE : METHODE_SETTING_YANN.md + STRATEGIE_YANN.md. Rien d'autre (pas l'ancien moteur SKILL_SETTING_DM.md/TEASING_METHODE_DM.md, pas de benchmark, pas Enzo Racine).

Réponds TOUJOURS dans ce format, court, sans pavé :
1. Phase actuelle (1 ligne)
2. Blocage (1-2 lignes)
3. Objectif du prochain message (1 ligne)
4. Éléments à mettre dedans (bullets courts et concrets — pas la phrase rédigée)

Tu ne rédiges jamais le message à ma place, sauf si je le demande explicitement — dans ce cas, dis "je te la rédige" et écris-la directement, sans résistance.

[Prospect : Prénom, headline si connu]
[Conversation collée ci-dessous]
```
...
Réflexes obligatoires à chaque post collé pour commentaire, sans que je le redemande : produis les propositions dans la conversation, en bloc code. **Aucun archivage pour les commentaires** : ni ARCHIVE_CONTENU.md, ni JOURNAL.md, ni commit/push — ça ne concerne que les posts et le setting DM (règle tranchée définitivement le 18 août 2026, annule la réactivation du 17 août 2026 qui avait remis l'archivage pour les commentaires punch ; confirme la décision initiale du 27 juillet 2026 qui les excluait déjà). Si je signale un problème sur une proposition, corrige directement le document concerné (SKILL_COMMENTAIRES_PUNCH.md ou SKILL_CONTENU_LINKEDIN.md Module 2 selon le cas, même logique d'itération que le setting DM), puis commit + push cette correction de skill (seul cas où un commit/push a lieu dans ce pôle).
