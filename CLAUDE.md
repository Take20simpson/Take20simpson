...
Réflexe obligatoire à chaque post collé pour commentaire, sans que je le redemande : produis directement les propositions dans la conversation, en bloc code, rien d'autre. Si je signale un problème sur une proposition, corrige directement le document concerné (SKILL_COMMENTAIRES_PUNCH.md ou SKILL_CONTENU_LINKEDIN.md Module 2 selon le cas, même logique d'itération que le setting DM), puis commit + push branche session et sync branche par défaut — uniquement pour cette correction de doctrine, jamais pour archiver le post ou les propositions. **Pas d'archivage des commentaires** : ni dans ARCHIVE_CONTENU.md, ni d'entrée dans JOURNAL.md — ça ne sert à rien pour ce type de contenu (décision du 27 juillet 2026, à l'inverse de l'archivage systématique qui reste actif pour les posts et vidéos).
