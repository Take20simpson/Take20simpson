...
- Statut : R1 fait le 16 septembre 2026 (visio). **R2 : tension de date non tranchée — `QUESTIONNAIRE_ONBOARDING.md`/`QUESTIONS_YANN_LENI.md`/`CONTRAT_ACCOMPAGNEMENT.md` indiquaient "dimanche 21 septembre 11h", mais le call vocal avec Yann du 19 septembre 2026 (voir `STRATEGIE_YANN.md`) situe le R2 "demain, 11h" — donc le samedi 20 septembre. À confirmer directement avec Matthias avant de trancher silencieusement l'une des deux dates.** Conversation menée par Matthias directement (post-31 août, méthode Yann, hors moteur IA `SKILL_SETTING_DM.md`).
...
- Prochaine étape probable : préparer le R2 (date à confirmer, voir tension ci-dessus) — priorité : structurer une méthode pour définir cible/offre/prix d'un dev freelance sans niche ni offre packagée, avant de proposer l'accompagnement au R2.
- **Plan de closing R2 validé avec Yann le 19 septembre 2026 (voir `STRATEGIE_YANN.md`, "Notes opérationnelles annexes") :**
  1. Qualifier au niveau du prix : demander ce qu'il peut lâcher maintenant, puis creuser au-delà du premier chiffre annoncé (checker ce qu'il a réellement disponible, quand rentre son MRR ConvoyDoc). Ne pas viser mécaniquement 666€ (tiers de l'échéancier) — viser 800€ upfront, 700€ en repli. Ce qui compte est le montant de l'engagement upfront, pas le nombre d'échéances.
  ...
  3. Signature du contrat obligatoire en direct pendant l'appel, même si Léni n'a pas la totalité de la somme disponible immédiatement.
  4. Annoncer que le démarrage concret (première session d'onboarding) a lieu la semaine suivante (à partir du lundi 22 ou 29 selon la date réelle du R2) plutôt qu'immédiatement — motif : Matthias monte en charge de clients, veut faire un onboarding correct. Le paiement upfront et la signature restent immédiats ; seul le démarrage opérationnel est décalé.
  5. Pendant l'onboarding, resserrer l'offre de Léni sur les SaaS spécifiquement (pas apps/sites/tout) — s'appuyer sur le cas Julien Robert (client de l'équipe Yann, 27 000€ en 50 jours en ciblant les SaaS) ; Yann propose de mettre Matthias en contact avec Alban pour creuser comment ce ciblage a été fait.
  6. Mettre en place la vidéo hebdo obligatoire (bilan chaque vendredi, même système que le rituel Matthias/Yann) — prévoir un rappel/alarme pour que Léni la fasse sans avoir à le redemander.
