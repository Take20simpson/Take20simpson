...
- Statut : Phase 3 avancée, la relance du 27/07 a fonctionné et a fait remonter un vrai pain point (ghosting après devis)
- Dernier point : dev WordPress passionnée de backend, 3 clients actifs (au 10/07), sourcing bouche-à-oreille + Malt occasionnel (évité à cause de la concurrence à prix cassés). A confirmé subir régulièrement des prospects "c'est wordpress ça doit pas être cher" (50/50 sur l'année). Le devis du lead chaud (référence familiale, mockup envoyé) a bien été envoyé mais décision reportée à septembre (elle part en vacances en août). Elle a spontanément révélé un second cas, plus frais et plus frustrant : un devis Malt envoyé début juillet, plusieurs relances de sa part (lundi, puis vendredi suivant), toujours zéro réponse depuis
- Patterns utilisés : Règle 6 (quantification, cas de référence documenté — a aussi servi de contre-exemple pour la Règle 7, sur-enchaînement de questions sèches ressenti comme un interrogatoire), Règle 1 (rebond mots exacts), forme "callback running gag co-créé" (relance du 27/07, réussie). Réponse au 27/07 = Règle 3 (valider la frustration du silence après relances multiples, pas générique) + Règle 14 (signal marché : le silence total post-devis, pattern observé chez d'autres indés en sur-mesure, souvent pire qu'un refus) + Règle 6 (quantifier la fréquence de ce silence sur ses derniers devis, formulation variée pour ne pas répéter "quelle proportion" déjà utilisé plus tôt dans la même conversation)
- Prochaine étape probable : si elle quantifie la fréquence du ghosting, évaluer si c'est un vrai pattern récurrent (frein de closing/suivi) plutôt qu'un cas isolé — pourrait justifier une Phase 4 sur l'impact réel (temps perdu, revenu manqué)
- Dernière mise à jour : 27 juillet 2026
