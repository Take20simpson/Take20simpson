...
- **Repartition des taches (precise par Matthias le 27 juillet) :** il se positionne naturellement sur le cote operationnel lourd — creation, animation de communaute, onboarding en visio par membre, definition des regles/subtilites. A poser explicitement face a Maryline pour diviser qui fait quoi (impact direct sur le point 3 ci-dessous : onboarding visio par membre n'est pas neutre en temps si le pod grossit)
- **Points structurels ouverts, a trancher a l'appel avec Maryline (prevu ~29 juillet, 2 jours apres cette session) :**
  1. **Clarte offre/cible :** Matthias affirme que son offre et sa cible n'ont "rien a voir" avec celles de Maryline. A objectiver avant l'appel plutot qu'a assumer — `JOURNAL.md` decrit l'offre de Maryline en termes proches du positionnement de Matthias (aide freelances a signer plus/mieux). Exercice propose par Claude, valide par Matthias : une phrase chacun ("moi je fais X pour Y" / "elle fait Z pour W"), comparees cote a cote avant l'appel. Si distinctes, le risque de chevauchement/poaching entre pipelines tombe ; sinon, ca montre ou poser la frontiere
  2. **Nom presentable cote prospect (valide le 27 juillet) :** ne jamais utiliser le mot "pod" face a un prospect (connotation growth-hacking/manipulatoire) — trouver un nom du type "cercle prive"/"groupe d'entraide"
  3. **Charge de bande passante :** Matthias a deja mis la prospection en pause le 24 juillet faute de bande passante (voir tension non tranchee, Principe #4 plus haut). Le pod serait une 3e initiative parallele (avec le contenu) — budget-temps a definir avant lancement pour ne pas retomber dans le pattern de dispersion identifie (`CLAUDE.md` Partie 1, 3 erreurs sur 4 ans). Precision de Matthias (27 juillet) : il lui reste plusieurs heures libres apres ses taches quotidiennes, mais reconnait le risque de dispersion. Recommandation Claude en attente de validation : fixer un chiffre d'heures/semaine dedie au pod avant l'appel plutot que de caler sur le temps disponible du moment (echo au principe mode automatique → mode intention, `CLAUDE.md` Partie 1)
